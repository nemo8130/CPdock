...
$path=$ARGV[2] || die "Enter full-path of the working directory\n";
...
open (INP,"<inp12map.res") || die "inp12map.res not found\n";
@rtot=<INP>;
close INP;
$irtot=@rtot;
...
open (INP1,"<$fort14");
open (INP2,"<$fort15");
open (MAP,"<$path/reso2n.map");

open (OUT1,">$path/intf1.res");
open (OUT2,">$path/intf2.res");
open (RAS1,">$path/ras1.select");
open (RAS2,">$path/ras2.select");
...
$fracI=sprintf("%12.5f",(($i1c+$i2c)/$irtot));
printf "#fracI: %10.5f\n",$fracI;

#============================================================================ 06/07/2021 =====
open(APP,"<fort.16") || die "fort.16 not found\n";
@str=<APP>;
chomp $str[0];
chomp $str[1];
close APP;
...
print "$str[0]     |fracI-----|\n";
print "$str[1]     $fracI\n";
print APP "$str[0]     |fracI-----|\n";
print APP "$str[1]     $fracI\n";
close APP;
#=============================================================================================
